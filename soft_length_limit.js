(function ($) {
  Drupal.behaviors.softLengthLimit = {
    attach: function (context, settings) {
      ...
      //Adds the soft limit counter to fields with a maxlength defined, if not disabled
      if(Drupal.settings.soft_length_limit === undefined ||
        ...
        var excluded=[];
        //Gets a list of selectors for elements that should be excluded even though they have a max length
      ...
      $('.soft-length-limit').each(function(index){
        ...
        //Used for automatically moving the tooltip when resizing the text area
      ...
      $('.soft-length-limit').bind('textchange', function(event, prevText){
        ...
        //Removes the "exceeded" class if length is not exceeded anymore
